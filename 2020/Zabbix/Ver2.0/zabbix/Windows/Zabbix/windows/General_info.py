...
def get_cpu_name():
	conent=os.popen("wmic cpu get name /value").readlines()
	CPU_name=""
	for index in range(len(conent)):
		if conent[index].strip()=="":
			...
			CPU_name=CPU_name+re.split(r'[=]',conent[index])[1].strip()+';	'
	return(CPU_name)
	
def get_cpu_QTY():
	conent=os.popen("wmic cpu get DeviceID /value").readlines()
	CPU_QTY=0
	for index in range(len(conent)):
		if conent[index].strip()=="":
			continue
...
def get_CPU_NumberOfCores():
	conent=os.popen("wmic cpu get NumberOfCores /value").readlines()
	CPU_NumberOfCores=0
	for index in range(len(conent)):
		if conent[index].strip()=="":
			continue
			...
			CPU_NumberOfCores=CPU_NumberOfCores+int(re.split(r'[=]',conent[index])[1].strip())
